...
CREATE DATABASE INVENTORY_UTT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;;
USE INVENTORY_UTT;
...
ALTER TABLE INVENTORY ADD PRIMARY KEY PK_INVENTORY (INVENTORY_ID),
            ...
            ADD FOREIGN KEY FK_INVENTORY_LOCATION (LOCATION_CODE) REFERENCES ASSET (ASSET_CODE),
...
CREATE TABLE LOAN(
    ...
    RESPONSIBLE VARCHAR(450) NULL,
    RESPONSIBLE_KEY VARCHAR(450) NULL,
    RESPONSIBLE_CONTACT1 VARCHAR(450) NULL,
    RESPONSIBLE_CONTACT2 VARCHAR(450) NULL
);
...
-- * FN IS INVENTORY USED
DROP FUNCTION IF EXISTS IS_INVENTORY_USED;
DELIMITER //
CREATE FUNCTION IS_INVENTORY_USED (ID INT)
...
BEGIN
    RETURN EXISTS (
        SELECT
            *
        FROM
            INVENTORY I JOIN LOAN_DTL LD ON I.INVENTORY_ID = LD.INVENTORY_ID
            JOIN LOAN L ON L.LOAN_ID = LD.LOAN_ID
        WHERE   I.INVENTORY_ID = ID
        AND     L.RETURN_DT IS NULL
